...
import DatePicker from '../UI/DatePicker'; // This is for <input type="date">
import OrderItemRow from './OrderItemRow';
import Modal from '../UI/Modal';
import { createNewOrder, updateExistingOrder, fetchCustomers } from '../../services/api';
import { Plus, Save, UserPlus, Search, UserCheck, Edit2, CheckSquare, XSquare, AlertTriangle } from 'lucide-react'; // All icons used
import { format, parseISO, isValid as isValidDate, setHours, setMinutes, setSeconds } from 'date-fns';
import Spinner from '../UI/Spinner'; // For isSearchingCustomers

// Mock data (to be replaced with dynamic data from settings/backend eventually)
...
const CreateOrderForm = ({ initialOrderData, isEditMode = false }) => {
    ...
    const [isLoading, setIsLoading] = useState(false); // For API submission loading state
    ...
    // Effect to populate form when initialOrderData (for editing) is provided
    useEffect(() => {
        if (isEditMode && initialOrderData) {
            if (initialOrderData.customer) { // If customer object is nested (populated)
                setCustomerInput({
            ...
            } else if (initialOrderData.customerId) { // If only customerId is flat on initialOrderData
                 // This case might need a separate fetch for customer details if not all are present
                 // For now, assume basic details might be there or form starts somewhat blank for customer if only ID given
                setSelectedCustomerId(initialOrderData.customerId);
                setCustomerSelectionMode('existing');
                // Potentially set customerInput from flat initialOrderData fields if they exist
                setCustomerInput({
                    name: initialOrderData.customerName || '', phone: initialOrderData.customerPhone || '',
                    email: initialOrderData.customerEmail || '', address: initialOrderData.customerAddress || '',
                });
            }
            ...
            // Reset form for new order creation
    ...
    }, [initialOrderData, isEditMode, defaultPickupDateOnly, defaultPickupTime]); // Added isEditMode
    ...
    const handleReviewOrder = (e) => {
        ...
        if (customerSelectionMode === 'existing' && !selectedCustomerId && isEditMode && !initialOrderData?.customer?._id && !initialOrderData?.customerId) {
             // If editing, but somehow no customer ID is present from initial data (should not happen ideally)
             setError('Editing an order requires an associated customer ID.'); return;
        } else if (customerSelectionMode === 'existing' && !selectedCustomerId && !isEditMode) {
             setError('Please select an existing customer or switch to create new.'); return;
        }
        ...
        setOrderDataForReview({
            isEditMode,
            customer: { ...customerInput, id: selectedCustomerId }, // selectedCustomerId is the actual customer _id
            ...
            calculatedDiscountAmount, finalTotalAmount, amountPaid: parseFloat(amountPaid) || 0, notes,
        });
    ...
    const handleConfirmAndCreateOrder = async () => {
        ...
        if (orderDataForReview.customerSelectionMode === 'existing' && orderDataForReview.customer.id) {
            ...
            // Send details for potential update only if different from what might be on initialOrderData (if editing)
            // This part can be complex; for now, always sending. Backend should be idempotent.
        ...
        try {
            ...
            navigate(isEditMode ? `/orders/${response.data._id}` : '/'); // Navigate to details or dashboard
        } catch (err) { 
            ...
            <form onSubmit={handleReviewOrder} className="space-y-8 divide-y divide-apple-gray-200 dark:divide-apple-gray-700">
                ...
                <div className="pt-8">
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-y-4 gap-x-6 sm:grid-cols-6">
                        <div className="sm:col-span-3"><Input label="Subtotal (FCFA)" id="subTotal" type="number" value={subTotal.toFixed(2)} readOnly className="bg-apple-gray-100 dark:bg-apple-gray-800 cursor-default" /></div>
                        <div className="sm:col-span-3"> <Select label="Discount Type" id="discountType" value={discountType} onChange={(e) => { setDiscountType(e.target.value); if (e.target.value === 'none') setDiscountValueState('0'); }} options={[ { value: 'none', label: 'No Discount' }, { value: 'percentage', label: 'Percentage (%)' }, { value: 'fixed', label: 'Fixed Amount ($)' }]} /> </div>
                        {discountType !== 'none' && ( <div className="sm:col-span-3"> <Input label={`Discount Value ${discountType === 'percentage' ? '(%)' : '($)'}`} id="discountValueState" type="number" value={discountValueState} onChange={(e) => setDiscountValueState(e.target.value)} min="0" step={discountType === 'percentage' ? '0.1' : '0.01'} /> </div> )}
                        <div className="sm:col-span-3"><Input label="Calculated Discount (FCFA)" id="calculatedDiscountDisplay" type="text" value={calculatedDiscountAmount.toFixed(2)} readOnly className="bg-apple-gray-100 dark:bg-apple-gray-800 cursor-default" /></div>
                        <div className="sm:col-span-3"><Input label="Final Total Amount (FCFA)" id="finalTotalAmount" type="text" value={finalTotalAmount.toFixed(2)} readOnly className="font-semibold bg-apple-gray-100 dark:bg-apple-gray-800 cursor-default" /></div>
                        <div className="sm:col-span-3"><Input label="Amount Paid (FCFA)" id="amountPaid" name="amountPaid" type="number" value={amountPaid} onChange={(e) => setAmountPaid(e.target.value)} step="0.01" min="0" /></div>
            ...
            {showConfirmationModal && orderDataForReview && (
                <Modal isOpen={showConfirmationModal} onClose={() => { if (!isLoading) setShowConfirmationModal(false); }} title={isEditMode ? "Confirm Order Changes" : "Confirm New Order"} size="2xl">
                    <div className="space-y-4 text-sm max-h-[70vh] overflow-y-auto p-1 custom-scrollbar">
                        {/* ... Modal Content (Customer, Items, Summary with discount) ... */}
                        ...
                        <div className="p-3 border rounded-apple-md dark:border-apple-gray-700 bg-apple-gray-50 dark:bg-apple-gray-800/30">
                            ...
                            {orderDataForReview.items.length > 0 ? orderDataForReview.items.map((item, index) => (
                                ...
                                </div> )) : <p className="italic">No items.</p>}
                        </div>
                        <div className="p-3 border rounded-apple-md dark:border-apple-gray-700 bg-apple-gray-50 dark:bg-apple-gray-800/30">
                    ...
                    <div className="mt-6 pt-4 border-t dark:border-apple-gray-700 flex flex-wrap justify-end gap-3">
                        ...
                        <Button variant="primary" onClick={handleConfirmAndCreateOrder} isLoading={isLoading} iconLeft={<Save size={16}/>}>
                            {isEditMode ? "Confirm & Save" : "Confirm & Create"}
                        </Button>
                    </div>
                </Modal>
            )}
