...
const CreateOrderForm = ({ initialOrderData, isEditMode = false }) => {
    ...
    useEffect(() => {
        try {
            console.log("[CreateOrderForm] useEffect for initialOrderData. isEditMode:", isEditMode, "InitialData available:", !!initialOrderData);
            if (isEditMode && initialOrderData) {
                console.log("[CreateOrderForm] Populating form for EDIT mode with data:", initialOrderData);
                if (initialOrderData.customer) {
                    ...
                    console.warn("[CreateOrderForm] Edit mode, but customer data is incomplete in initialOrderData.");
                    setCustomerSelectionMode('new'); // Fallback if customer info is missing for edit
                }
                ...
                setError('');
            } else { // CREATE NEW ORDER MODE or if initialData is null
            ...
            setError(`Initialization Error: ${e.message}. Check console for details.`);
        }
    }, [initialOrderData, isEditMode, defaultPickupDateOnly, defaultPickupTime]); // Dependencies

    // --- Event Handlers ---
    const handleCustomerInputChange = (e) => {
        try { setCustomerInput({ ...customerInput, [e.target.name]: e.target.value }); }
        catch (e) { console.error("!!! FRONTEND ERROR in handleCustomerInputChange:", e); }
    };
    const handleCustomerSearchChange = async (e) => { /* ... as before ... */ };
    const handleSelectCustomer = (customer) => { /* ... as before ... */ };
    const handleCreateNewCustomerToggle = () => { /* ... as before ... */ };
    const handleItemChange = (id, field, value) => {
        try {
            const newItems = items.map(item => item.id === id ? { ...item, [field]: field === 'quantity' ? (parseInt(value, 10) || 1) : value } : item );
            setItems(newItems);
        } catch (e) { console.error("!!! FRONTEND ERROR in handleItemChange:", e); }
    };
    const handleAddItem = () => { try { setItems([...items, { id: Date.now(), itemType: '', serviceType: 'wash', quantity: 1, specialInstructions: '' }]); } catch (e) { console.error("!!! FRONTEND ERROR in handleAddItem:", e); }};
    const handleRemoveItem = (id) => { try { setItems(items.filter(item => item.id !== id)); } catch (e) { console.error("!!! FRONTEND ERROR in handleRemoveItem:", e); }};

    // --- Calculations ---
    const calculateItemPrice = useCallback((item) => {
        // Ensure this function is robust and doesn't throw errors
        // For example, check if item and item.serviceType exist before accessing them
        let pricePerUnit = 0;
        const qty = parseInt(item?.quantity, 10) || 0; // Added optional chaining and default
        if (qty <= 0) return 0;
        ...
        else if (itemType === 'Coat') pricePerUnit *= 1.5;
    ...
    useEffect(() => {
        ...
        } catch(e) { console.error("!!! FRONTEND ERROR in useEffect [subTotal calculation]:", e); }
    ...
    useEffect(() => {
        ...
        } catch(e) { console.error("!!! FRONTEND ERROR in useEffect [discount calculation]:", e); }
    ...
    const handleReviewOrder = (e) => {
        ...
        try {
            ...
            if (!isValidDate(combinedDateTime)) { throw new Error('Invalid pickup date after parsing.'); }
            ...
            console.log("[CreateOrderForm] Data for review modal:", JSON.stringify(dataToReview, null, 2));
            ...
            setError(`Error preparing order for review: ${e.message}. Check console.`);
        }
    };
    ...
    const handleConfirmAndCreateOrder = async () => {
        ...
        try {
            ...
            setError(errMsg);
            console.error("[CreateOrderForm] Order Submission Error (Axios Error Object):", err.response || err.message || err);
    ...
    return (
        <>
            <form onSubmit={handleReviewOrder} className="space-y-8 divide-y divide-apple-gray-200 dark:divide-apple-gray-700">
                ...
                <div className="pt-2"> <h3 className="text-xl font-semibold leading-7 text-apple-gray-900 dark:text-apple-gray-100">Customer Information</h3> <div className="mt-2 mb-4 flex space-x-4"> <Button type="button" onClick={() => { setCustomerSelectionMode('existing'); setSelectedCustomerId(null); setCustomerInput({ name: '', phone: '', email: '', address: '' }); }} variant={customerSelectionMode === 'existing' ? 'primary' : 'secondary'} iconLeft={<Search size={16}/>}>Existing Customer</Button> <Button type="button" onClick={handleCreateNewCustomerToggle} variant={customerSelectionMode === 'new' ? 'primary' : 'secondary'} iconLeft={<UserPlus size={16}/>}>New Customer</Button> </div> {customerSelectionMode === 'existing' && ( <div className="space-y-4 mb-4"> <Input label="Search Customer (Name/Phone)" id="customerSearch" value={customerSearchQuery} onChange={handleCustomerSearchChange} placeholder="Type to search..." prefixIcon={isSearchingCustomers ? <Spinner size="sm"/> : <Search size={16}/>} /> {searchedCustomers.length > 0 && ( <ul className="max-h-48 overflow-y-auto border border-apple-gray-300 dark:border-apple-gray-700 rounded-apple-md divide-y divide-apple-gray-200 dark:divide-apple-gray-700 bg-white dark:bg-apple-gray-800"> {searchedCustomers.map(cust => ( <li key={cust._id} onClick={() => handleSelectCustomer(cust)} className="p-3 hover:bg-apple-gray-100 dark:hover:bg-apple-gray-700/50 cursor-pointer"> <p className="font-medium text-apple-gray-800 dark:text-apple-gray-200">{cust.name}</p> <p className="text-sm text-apple-gray-500 dark:text-apple-gray-400">{cust.phone} {cust.email && `- ${cust.email}`}</p> </li> ))} </ul> )} {selectedCustomerId && <p className="mt-2 text-sm text-apple-green flex items-center"><UserCheck size={16} className="mr-1"/> Selected: {customerInput.name} ({customerInput.phone})</p>} </div> )} <div className={`mt-2 grid grid-cols-1 gap-y-4 gap-x-6 sm:grid-cols-6 ${customerSelectionMode === 'existing' && !selectedCustomerId && !isEditMode && 'opacity-60 pointer-events-none'}`}> <div className="sm:col-span-3"> <Input label="Full Name*" id="customerNameInput" name="name" value={customerInput.name} onChange={handleCustomerInputChange} required={customerSelectionMode === 'new'} /> </div> <div className="sm:col-span-3"> <Input label="Phone Number*" id="customerPhoneInput" name="phone" type="tel" value={customerInput.phone} onChange={handleCustomerInputChange} required={customerSelectionMode === 'new'} /> </div> <div className="sm:col-span-3"> <Input label="Email Address (Optional)" id="customerEmailInput" name="email" type="email" value={customerInput.email} onChange={handleCustomerInputChange} /> </div> <div className="sm:col-span-3"> <Input label="Address (Optional)" id="customerAddressInput" name="address" value={customerInput.address} onChange={handleCustomerInputChange} /> </div> </div> </div>
                <div className="pt-8"> <h3 className="text-xl font-semibold leading-7 text-apple-gray-900 dark:text-apple-gray-100">Clothing Items</h3> <div className="mt-6 space-y-4"> {items.map((item, index) => { const itemPrice = calculateItemPrice(item); return ( <OrderItemRow key={item.id} item={item} index={index} onRemove={() => handleRemoveItem(item.id)} onChange={handleItemChange} itemTypes={MOCK_ITEM_TYPES} serviceTypes={MOCK_SERVICE_TYPES} calculatedPrice={itemPrice} /> ); })} <Button type="button" onClick={handleAddItem} variant="secondary" iconLeft={<Plus size={16}/>}>Add Item</Button> </div> </div>
                <div className="pt-8"> <h3 className="text-xl font-semibold leading-7 text-apple-gray-900 dark:text-apple-gray-100">Order Summary, Discount & Dates</h3> <div className="mt-6 grid grid-cols-1 gap-y-4 gap-x-6 sm:grid-cols-6"> <div className="sm:col-span-3"><Input label={`Subtotal (${currencySymbol})`} id="subTotal" type="text" value={subTotal.toFixed(2)} readOnly className="bg-apple-gray-100 dark:bg-apple-gray-800 cursor-default" /></div> <div className="sm:col-span-3"> <Select label="Discount Type" id="discountType" value={discountType} onChange={(e) => { setDiscountType(e.target.value); if (e.target.value === 'none') setDiscountValueState('0'); }} options={[ { value: 'none', label: 'No Discount' }, { value: 'percentage', label: 'Percentage (%)' }, { value: 'fixed', label: `Fixed Amount (${currencySymbol})` }]} /> </div> {discountType !== 'none' && ( <div className="sm:col-span-3"> <Input label={`Discount Value ${discountType === 'percentage' ? '(%)' : `(${currencySymbol})`}`} id="discountValueState" type="number" value={discountValueState} onChange={(e) => setDiscountValueState(e.target.value)} min="0" step={discountType === 'percentage' ? '0.1' : '0.01'} /> </div> )} <div className="sm:col-span-3"><Input label={`Calculated Discount (${currencySymbol})`} id="calculatedDiscountDisplay" type="text" value={calculatedDiscountAmount.toFixed(2)} readOnly className="bg-apple-gray-100 dark:bg-apple-gray-800 cursor-default" /></div> <div className="sm:col-span-3"><Input label={`Final Total Amount (${currencySymbol})`} id="finalTotalAmount" type="text" value={finalTotalAmount.toFixed(2)} readOnly className="font-semibold bg-apple-gray-100 dark:bg-apple-gray-800 cursor-default" /></div> <div className="sm:col-span-3"><Input label={`Advance Paid (${currencySymbol})`} id="amountPaid" name="amountPaid" type="number" value={amountPaid} onChange={(e) => setAmountPaid(e.target.value)} step="0.01" min="0" /></div> <div className="sm:col-span-3"><Input label={`Balance Due (${currencySymbol})`} id="balanceDueDisplay" type="text" value={balanceDue.toFixed(2)} readOnly className={`font-semibold cursor-default ${balanceDue > 0 ? 'text-apple-red dark:text-red-400' : 'text-apple-green dark:text-green-400'} bg-apple-gray-100 dark:bg-apple-gray-800`} /></div> <div className="sm:col-span-3"><DatePicker label="Expected Pickup Date*" id="expectedPickupDateOnly" value={expectedPickupDateOnly} onChange={(e) => setExpectedPickupDateOnly(e.target.value)} required min={format(new Date(), 'yyyy-MM-dd')} /></div> <div className="sm:col-span-3"><Input label="Expected Pickup Time*" id="expectedPickupTime" type="time" value={expectedPickupTime} onChange={(e) => setExpectedPickupTime(e.target.value)} required /></div> <div className="sm:col-span-6"> <label htmlFor="notes" className="block text-sm font-medium text-apple-gray-700 dark:text-apple-gray-300 mb-1">Order Notes (Optional)</label> <textarea id="notes" name="notes" rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} className="form-textarea block w-full sm:text-sm  border-apple-gray-300 focus:border-apple-blue focus:ring-apple-blue dark:bg-apple-gray-800 dark:border-apple-gray-700 dark:text-apple-gray-100 dark:focus:border-apple-blue rounded-apple shadow-apple-sm" /> </div> </div> </div>
                <div className="pt-6 flex items-center justify-end gap-x-3"> <Button type="button" variant="secondary" onClick={() => navigate(isEditMode && initialOrderData?._id ? `/orders/${initialOrderData._id}` : '/')} disabled={isLoading}>Cancel</Button> <Button type="submit" variant="primary" isLoading={isLoading} iconLeft={<CheckSquare size={18}/>}> {isEditMode ? 'Review Changes' : 'Review Order'} </Button> </div>
            </form>
            ...
            {showConfirmationModal && orderDataForReview && (
                <Modal isOpen={showConfirmationModal} onClose={() => { if (!isLoading) setShowConfirmationModal(false); }} title={isEditMode ? "Confirm Order Changes" : "Confirm New Order"} size="2xl">
                    <div className="space-y-4 text-sm max-h-[70vh] overflow-y-auto p-1 custom-scrollbar">
                        ...
                        <div className="p-3 border rounded-apple-md dark:border-apple-gray-700 bg-apple-gray-50 dark:bg-apple-gray-800/30"> <h4 className="font-medium mb-2 text-apple-gray-700 dark:text-apple-gray-200">Items:</h4> {orderDataForReview.items.length > 0 ? orderDataForReview.items.map((item, index) => { const itemPriceInReview = calculateItemPrice(item); return ( <div key={index} className={`mb-2 pb-2 ${index < orderDataForReview.items.length - 1 ? 'border-b dark:border-apple-gray-700/50' : ''}`}> <div className="flex justify-between"> <span><strong>{item.quantity || 0}x {item.itemType || <span className="italic text-apple-red">Item type missing</span>}</strong> - Service: {item.serviceType || <span className="italic text-apple-red">Service missing</span>}</span> <span className="font-medium">{currencySymbol}{itemPriceInReview.toFixed(2)}</span> </div> {item.specialInstructions && <p className="text-xs italic text-apple-gray-600 dark:text-apple-gray-400">Instructions: {item.specialInstructions}</p>} </div> ); }) : <p className="italic text-apple-gray-500">No items added.</p>} </div>
                        <div className="p-3 border rounded-apple-md dark:border-apple-gray-700 bg-apple-gray-50 dark:bg-apple-gray-800/30"> <h4 className="font-medium mb-1 text-apple-gray-700 dark:text-apple-gray-200">Summary:</h4> <p><strong>Subtotal:</strong> {currencySymbol}{orderDataForReview.subTotalAmount.toFixed(2)}</p> {orderDataForReview.discountType !== 'none' && orderDataForReview.calculatedDiscountAmount > 0 && ( <p><strong>Discount ({orderDataForReview.discountType === 'percentage' ? `${orderDataForReview.discountValue}%` : `${currencySymbol}${orderDataForReview.discountValue.toFixed(2)}`}):</strong> -{currencySymbol}{orderDataForReview.calculatedDiscountAmount.toFixed(2)}</p> )} <p className="text-base font-semibold"><strong>Final Total:</strong> {currencySymbol}{orderDataForReview.finalTotalAmount.toFixed(2)}</p> <p><strong>Advance Paid:</strong> {currencySymbol}{orderDataForReview.amountPaid.toFixed(2)}</p> <p className={`font-semibold ${orderDataForReview.balanceDue > 0 ? 'text-apple-red' : 'text-apple-green'}`}> <strong>Balance Due:</strong> {currencySymbol}{orderDataForReview.balanceDue.toFixed(2)} </p> <p><strong>Expected Pickup:</strong> {orderDataForReview.expectedPickupDate ? format(parseISO(orderDataForReview.expectedPickupDate), 'MMM d, yyyy, h:mm a') : <span className="italic text-apple-red">Missing</span>}</p> {orderDataForReview.notes && <p className="mt-1"><strong>Order Notes:</strong> <span className="block whitespace-pre-wrap">{orderDataForReview.notes}</span></p>} </div>
