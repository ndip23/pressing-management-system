...
import { format, parseISO, isValid as isValidDate } from 'date-fns'; // Added isValid
import Spinner from '../UI/Spinner';

// --- Mock Data (Replace with dynamic data from settings/backend eventually) ---
...
const CreateOrderForm = ({ initialOrderData, isEditMode = false }) => {
    ...
    useEffect(() => {
        if (initialOrderData) {
            ...
            const initialExpectedDate = initialOrderData.expectedPickupDate ? format(parseISO(initialOrderData.expectedPickupDate), 'yyyy-MM-dd') : defaultPickupDate;
            setExpectedPickupDate(initialExpectedDate);
            setTotalAmount(initialOrderData.totalAmount || 0);
            setAmountPaid(initialOrderData.amountPaid || 0);
            setNotes(initialOrderData.notes || '');
        } else {
            // Reset to defaults if not edit mode or no initial data
    ...
    }, [initialOrderData, defaultPickupDate]); // Added defaultPickupDate dependency
    ...
    const handleCustomerSearchChange = async (e) => {
        ...
        if (query.length > 1) {
            ...
            } catch (err) { setSearchedCustomers([]); } finally { setIsSearchingCustomers(false); }
    ...
    const handleCreateNewCustomerToggle = () => {
        setCustomerSelectionMode('new'); setSelectedCustomerId(null);
    };
    ...
    const handleReviewOrder = (e) => {
        ...
        if (!expectedPickupDate || !isValidDate(parseISO(expectedPickupDate))) {
            setError('A valid expected pickup date is required.'); return;
        }
    ...
    const handleConfirmAndCreateOrder = async () => {
        ...
        const payload = {
            ...
            totalAmount: orderDataForReview.totalAmount,
        ...
        try {
            ...
            setError(errMsg); console.error(err);
        ...
        <>
            <form onSubmit={handleReviewOrder} className="space-y-8 divide-y divide-apple-gray-200 dark:divide-apple-gray-700">
                ...
                <div className="pt-8">
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-y-4 gap-x-6 sm:grid-cols-6">
                        ...
                        <div className="sm:col-span-3">
                            <Input label="Total Amount ($)" id="totalAmount" type="number" value={totalAmount.toFixed(2)} readOnly className="bg-apple-gray-100 dark:bg-apple-gray-800 cursor-default" />
                        </div>
            ...
            {showConfirmationModal && orderDataForReview && (
                <Modal
                    ...
                    size="2xl"
                >
                    <div className="space-y-4 text-sm max-h-[70vh] overflow-y-auto p-1 custom-scrollbar"> {/* Added padding & scroll */}
                        ...
                        <div className="p-3 border rounded-apple-md dark:border-apple-gray-700 bg-apple-gray-50 dark:bg-apple-gray-800/30">
                            ...
                            <p><strong>Expected Pickup:</strong> {orderDataForReview.expectedPickupDate ? format(parseISO(orderDataForReview.expectedPickupDate + 'T00:00:00Z'), 'MMM d, yyyy') : <span className="italic text-apple-red">Date Missing</span>}</p>
                            <p><strong>Amount Paid:</strong> ${orderDataForReview.amountPaid.toFixed(2)}</p>
                            <p className="text-base font-semibold"><strong>Total Amount:</strong> ${orderDataForReview.totalAmount.toFixed(2)}</p>
                        ...
                        <p className="pt-2 text-center font-semibold text-apple-gray-700 dark:text-apple-gray-200">
                            Is all information correct and ready to proceed?
                        </p>
                    </div>
                    <div className="mt-6 pt-4 border-t dark:border-apple-gray-700 flex flex-wrap justify-end gap-3"> {/* Flex-wrap for smaller screens */}
                        <Button variant="secondary" onClick={handleEditFromReview} iconLeft={<Edit2 size={16}/>} disabled={isLoading}>Edit Details</Button>
                        <Button variant="ghost" onClick={() => setShowConfirmationModal(false)} disabled={isLoading} className="text-apple-red">Cancel & Clear</Button> {/* Changed "Cancel Order Creation" to more generic */}
