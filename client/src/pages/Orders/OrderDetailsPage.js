...
import { ArrowLeft, Edit3, Printer, DollarSign, MessageSquare, AlertTriangle, CheckCircle2, Clock3 } from 'lucide-react';
...
const calculateItemPriceForReceipt = (item) => {
    ...
    switch (item.serviceType) {
        case 'dry clean': pricePerUnit = 3000; break;
        case 'wash & iron': pricePerUnit = 2000; break;
        case 'iron': pricePerUnit = 500; break;
        case 'wash': pricePerUnit = 1000; break;
        case 'special care': pricePerUnit = 5000; break;
        default: pricePerUnit = 2;
    }
...
const OrderDetailsPage = () => {
    ...
    const handleSendNotification = async () => {
        if (!order?.customer || (!order.customer.email && !order.customer.phone)) {
            setActionError("Customer contact information is missing.");
            return;
        }
    ...
    const handleMarkOrderAsPaid = async () => {
        ...
        if (window.confirm(`Are you sure you want to mark order #${order.receiptNumber} as fully paid?`)) {
        ...
        <div className="space-y-6">
                ...
                {isOrderOverdue && (
                    <Card className="bg-red-100 dark:bg-red-900/40 border-2 border-red-400 dark:border-red-600 shadow-lg animate-pulse-slow">
                        <div className="flex items-center p-4">
                            ...
                            <div>
                                <h3 className="text-lg font-semibold text-red-700 dark:text-red-300">Order Overdue!</h3>
                                {order.expectedPickupDate && <p className="text-sm text-red-600 dark:text-red-500">Expected: {format(parseISO(order.expectedPickupDate), dateTimeFormat)}</p>}
                            </div>
                        </div>
                    </Card>
                )}
            ...
            <div id="printable-receipt-area" className="bg-white dark:bg-white p-4 sm:p-6 rounded-apple-lg shadow-apple-md print-receipt-layout-onscreen-view">
                ...
                {order.customer?.email && (
                    <div className="text-xs mb-4"><span className="font-semibold">Email:</span> {order.customer.email}</div>
                )}
                                    ...
                                    <tr key={item._id || index} className="border-b border-dotted border-gray-300">
                                        ...
                                        <td className="py-1.5 align-top">
                                            ...
                                            {item.specialInstructions && (
                                                <div className="text-[10px] text-gray-500 pl-2">↳ {item.specialInstructions}</div>
                                            )}
                                        </td>
                ...
                <div className="mt-4 pt-2 border-t border-dashed border-gray-400 text-xs space-y-0.5">
                    ...
                    {order.discountType !== 'none' && order.discountAmount > 0 && (
                        <div className="flex justify-between"><span className="font-medium">Discount ({order.discountType === 'percentage' ? `${order.discountValue}%` : 'Fixed'}):</span><span>-{currencySymbol} {(order.discountAmount || 0).toFixed(2)}</span></div>
                    )}
                ...
                {order.notes && (
                    <div className="mt-3 pt-2 border-t border-dashed border-gray-400">
                        <h4 className="font-semibold text-xs mb-0.5">Order Notes:</h4>
                        <p className="text-[10px] whitespace-pre-wrap">{order.notes}</p>
                    </div>
                )}
                ...
                <div className="lg:col-span-2 space-y-6">
                    <Card title="Order Details (Screen View)" contentClassName="p-0">
                        <dl className="divide-y divide-apple-gray-100 dark:divide-apple-gray-800">
                            ...
                            {order.discountType !== 'none' && order.discountAmount > 0 && (
                                <DetailItem label={`Discount (${order.discountType === 'percentage' ? `${order.discountValue}%` : 'Fixed'})`} value={`${currencySymbol}${(order.discountAmount || 0).toFixed(2)}`} className="text-orange-600 dark:text-orange-400" />
                            )}
                                ...
                                {order.items.map((item, index) => (
                                    <li key={item._id || index} className="py-3">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <p className="text-sm font-medium text-apple-gray-800 dark:text-apple-gray-200">{currencySymbol}{calculateItemPriceForReceipt(item).toFixed(2)}</p>
                                        </div>
