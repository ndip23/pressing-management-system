...
const DetailItem = ({ label, value, className = "", children }) => (
    <div className={`py-3 sm:grid sm:grid-cols-3 sm:gap-4 border-b border-apple-gray-100 dark:border-apple-gray-800 px-4 sm:px-6 last:border-b-0 ${className}`}>
        ...
        <dd className="mt-1 text-sm text-apple-gray-900 dark:text-apple-gray-100 sm:mt-0 sm:col-span-2">{children || value || <span className="italic">N/A</span>}</dd>
    </div>
);
...
const calculateItemPriceForReceipt = (item) => {
    ...
    if (qty <= 0) return 0;
    if (item.serviceType === 'dry clean') pricePerUnit = 3000; else if (item.serviceType === 'wash & iron') pricePerUnit = 2000;
    else if (item.serviceType === 'iron') pricePerUnit = 500; else if (item.serviceType === 'wash') pricePerUnit = 1000;
    else if (item.serviceType === 'special care') pricePerUnit = 5000; else pricePerUnit = 2;
    if (item.itemType === 'Suit') pricePerUnit *= 2; else if (item.itemType === 'Coat') pricePerUnit *= 1.5;
...
const OrderDetailsPage = () => {
    ...
    const loadOrder = useCallback(async (showLoadingSpinner = true) => {
        ...
        try { const { data } = await fetchOrderById(id); setOrder(data); }
        catch (err) { setError(err.response?.status === 404 ? 'Order not found.' : (err.response?.data?.message || 'Failed to fetch order details.')); }
    ...
    useEffect(() => {
        let timer;
        if (actionSuccess || actionError) { timer = setTimeout(() => { setActionSuccess(''); setActionError(''); }, 5000); }
    ...
    const handleUpdateStatus = async (newStatus) => {
        ...
        } catch (err) { setActionError(err.response?.data?.message || 'Error updating status.');
    ...
    const handleRecordPayment = async () => {
        ...
        if (isNaN(paymentAmount) || paymentAmount <= 0) { setActionError("Please enter a valid positive number."); return; }

        setIsUpdatingStatus(true);
        try {
            const { data: updatedOrder } = await recordPartialPaymentApi(order._id, { amount: paymentAmount, method: 'Cash' });
            setOrder(updatedOrder); setActionSuccess(`${currencySymbol}${paymentAmount.toFixed(2)} payment recorded.`);
        } catch (err) { setActionError(err.response?.data?.message || "Failed to record payment.");
        } finally { setIsUpdatingStatus(false); }
    };

    const handleMarkOrderAsPaid = async () => {
        if (!order || order.isFullyPaid || isMarkingPaid) return;
        if (window.confirm(`Mark order #${order.receiptNumber} as fully paid? This will settle the remaining balance.`)) {
            setIsMarkingPaid(true); setActionError(''); setActionSuccess('');
            try { const { data: updatedOrder } = await markOrderPaidApi(order._id); setOrder(updatedOrder); setActionSuccess(`Order #${updatedOrder.receiptNumber} marked as fully paid.`);
            } catch (err) { setActionError(err.response?.data?.message || 'Failed to mark order as paid.');
    ...
    const handleDeleteOrder = async () => { /* ... as before ... */ };
    ...
    const formatDateSafe = (dateString, formatString = dateTimeFormat) => {
        if (!dateString) return 'N/A';
        const date = parseISO(dateString);
        return isValidDate(date) ? format(date, formatString) : 'Invalid Date';
    };


    return (
        <div className="space-y-6">
            {/* Header and Messages (hidden on print) */}
            <div className="print-hide">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div className="flex items-center space-x-2"> {/*<Button variant="secondary" onClick={() => loadOrder(false)} iconLeft={<RefreshCw size={16}/>} isLoading={loading && !!order} disabled={loading && !!order}>Refresh</Button>*/} <Button variant="secondary" onClick={handlePrintReceipt} iconLeft={<Printer size={16}/>}>Print Receipt</Button> 
                    {/*<Link to={`/app/orders/${order._id}/edit`}><Button variant="primary" iconLeft={<Edit3 size={16}/>}>Edit Order</Button></Link> {user?.role === 'admin' && (<Button variant="danger" onClick={handleDeleteOrder} iconLeft={<Trash2 size={16}/>} isLoading={isDeleting} disabled={isDeleting}>Delete</Button>)}*/} </div>
                </div>
            ...
            {/* The main content is now inside a grid, and the printable receipt is its own separate element */}

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* --- On-Screen Details (Left and Center Columns) --- */}
                ...
                {/* --- On-Screen Actions (Right Column) --- */}
                <div className="lg:col-span-1 space-y-6 print-hide">
                    <Card title="Financial Summary" contentClassName="p-0">
                        <dl className="divide-y divide-apple-gray-100 dark:divide-apple-gray-800">
                            <DetailItem label="Subtotal" value={`${currencySymbol}${(order.subTotalAmount || 0).toFixed(2)}`} />
                            {order.discountType !== 'none' && order.discountAmount > 0 && ( <DetailItem label={`Discount`} value={`-${currencySymbol}${(order.discountAmount || 0).toFixed(2)}`} className="text-orange-600 dark:text-orange-400" /> )}
            ...
            {/* Printable Receipt Area (Only visible for printing due to CSS) */}
            <div id="printable-receipt-area" className="hidden print:block">
                <div className="text-center mb-6 company-info-print"> <h2 className="text-xl font-bold text-black">{companyInfo.name}</h2> <p className="text-xs text-gray-700">{companyInfo.address}</p> <p className="text-xs text-gray-700">Phone: {companyInfo.phone}</p> <hr className="my-3 border-dashed border-gray-400" /> </div>
