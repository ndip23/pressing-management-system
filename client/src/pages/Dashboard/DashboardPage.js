    ...
    const loadOrders = useCallback(async () => {
        ...
        try {
            ...
            setStats({
                ...
                pending: displayedOrders.filter(o => ['Pending', 'Processing'].includes(o.status)).length,
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <Link to="/orders/new">
                    <Button variant="primary" size="md" iconLeft={<PlusCircle size={18} />}>Create New Order</Button>
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <StatCard title="Total Orders" value={pagination.totalOrders} icon={<Shirt size={24} className="text-apple-blue" />} isLoading={loadingOrders && !orders.length && pagination.totalOrders === 0} />
                <StatCard title="Pending/Processing" value={stats.pending} icon={<Clock3 size={24} className="text-apple-orange" />} isLoading={loadingOrders && !orders.length} />
                <StatCard title="Ready for Pickup" value={stats.ready} icon={<CheckCircle2 size={24} className="text-apple-green" />} isLoading={loadingOrders && !orders.length} />
                <StatCard title="Overdue Orders" value={stats.overdue} icon={<AlertTriangle size={24} className="text-apple-red" />} colorClass={stats.overdue > 0 ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700' : ''} isLoading={loadingOrders && !orders.length} />
            </div>
