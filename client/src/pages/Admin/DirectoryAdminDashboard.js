...
import ListingFormModal from '../../components/Admin/ListingFormModal'; // Import the new modal
...
const DirectoryAdminDashboard = () => {
    ...
    const [editingListing, setEditingListing] = useState(null); // null for create, object for edit
    ...
    const handleFormSubmit = async (formData) => {
        ...
        setApiError('');
        try {
            if (editingListing) { // Update existing
            ...
            // Re-throw the error so the modal's internal state knows it failed
            throw err;
        } finally {
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full text-sm">
                                    ...
                                    <tbody className="divide-y divide-apple-gray-200 dark:divide-apple-gray-700">
                                        {listings.length === 0 && (
                                            <tr><td colSpan="5" className="text-center p-8 text-apple-gray-500">No manual listings found.</td></tr>
                                        )}
                                            ...
                                            <tr key={listing._id} className="hover:bg-apple-gray-50 dark:hover:bg-apple-gray-800/50">
                                                ...
                                                <td className="px-4 py-3 text-center">
                                                    <div className="flex justify-center space-x-2">
                                                        <Button size="sm" variant="ghost" onClick={() => handleOpenEditModal(listing)}><Edit3 size={16} /></Button>
                                                        <Button size="sm" variant="ghost" className="text-apple-red" onClick={() => handleDeleteListing(listing)}><Trash2 size={16} /></Button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
