...
const createOrder = asyncHandler(async (req, res) => {
    ...
    if (customerId) {
        ...
        const updateData = { name: customerName, phone: customerPhone, tenantId: tenantId };
        if (customerEmail !== undefined) updateData.email = customerEmail.toLowerCase();
        if (customerAddress !== undefined) updateData.address = customerAddress;
        
        customer = await Customer.findOneAndUpdate(
            { phone: customerPhone, tenantId: tenantId },
            { $set: updateData, $setOnInsert: { name: customerName, phone: customerPhone } },
...
// @desc    Mark an order as fully paid by settling the balance
// @route   PUT /api/orders/:id/mark-paid
// @access  Private
const markOrderAsPaid = asyncHandler(async (req, res) => {
    const order = await Order.findOne({ _id: req.params.id, tenantId: req.tenantId });
    if (!order) { res.status(404); throw new Error('Order not found.'); }
    if (order.isFullyPaid) { return res.json(order); } // Already paid, just return it
...
const recordPartialPayment = asyncHandler(async (req, res) => {
    ...
    if (newTotalPaid > order.totalAmount + 0.01) { res.status(400); throw new Error(`Payment of ${amount} exceeds balance due.`); }
    order.payments.push({ amount: parseFloat(amount), method: method || 'Cash', recordedBy: req.user.id });
    const updatedOrder = await order.save();
    const populatedOrder = await Order.findById(updatedOrder._id).populate('customer').populate('createdBy', 'username');
    res.json(populatedOrder);
});
